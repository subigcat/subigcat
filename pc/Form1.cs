            ...
            for (int i = 0; i < computer.Hardware.Length; i++)
            {
                if (computer.Hardware[i].HardwareType == HardwareType.GpuNvidia)
                {
                    ...
                    for (int j = 0; j < computer.Hardware[i].Sensors.Length; j++)
                    {
                        ...
                        if (computer.Hardware[i].Sensors[j].Name == "Power")
                        {
                            if (computer.Hardware[i].Sensors[j].Name == "GPU Power")
                            {
                                // gpupowershow.Text = computer.Hardware[i].Sensors[j].Value.ToString() + " W";
                            }
                        }
                        if (computer.Hardware[i].Sensors[j].SensorType == SensorType.Temperature)
                        {
                        ...
                        if (computer.Hardware[i].Sensors[j].SensorType == SensorType.Throughput)
                        {
                            if (computer.Hardware[i].Sensors[j].Name== "GPU PCIE Rx")
                            {
                                //gpupcierx.Text = computer.Hardware[i].Sensors[j].Value.ToString() + "MB/s";
                            }
                            if (computer.Hardware[i].Sensors[j].Name == "GPU PCIE Tx")
                            {
                                //gpupcietx.Text = computer.Hardware[i].Sensors[j].Value.ToString() + "MB/s";
                            }
                        }
